  ...
  return (
    <Box>
      <Paragraph title="ОСНОВНЫЕ УЧЕБНЫЕ РЕСУРСЫ">
        ...
        <SubPara>
          Также хорошим помощником станет книга «Олимпиадное программирование»
          от Антти Лааксонена. Помимо различных приемов проектирования
          алгоритмов в ней подробно описано, как готовиться к олимпиадам, какими
          качествами нужно обладать, чтобы добиться высоких результатов.
        </SubPara>
      </Paragraph>
      <Paragraph
      ...
      >
        ...
        <Paragraph title="2. СЛОЖНОСТЬ АЛГОРИТМОВ" id="big-o">
          <SubPara>
            ...
            решения проблемы. Обычно это измеряется в терминах временной
            сложности и пространственной сложности. Временная сложность - это
            количество времени, необходимое для завершения алгоритма, а
            пространственная сложность - это количество памяти, которое алгоритм
            использует.
          </SubPara>
          <SubPara>
            Big O нотация - это способ выражения временной сложности алгоритма в
            виде функции размера его входных данных. Она используется для
            описания наихудшего сценария, или верхней границы, временной
            сложности алгоритма. Например, если алгоритм занимает 2n + 5 шагов
            для завершения, мы бы выразили его временную сложность в виде
            большой O-нотации как O(n), где n представляет размер входных
            данных.
          </SubPara>
          <SubPara>
            ...
            размера входных данных. В целом, по мере роста размера входных
            данных мы хотим, чтобы временная сложность алгоритма росла медленнее
            всего, что означает, что мы хотим выбирать алгоритмы с меньшими
            классами временной сложности, когда это возможно.
          </SubPara>
          <SubPara>
            В целом, понимание сложности алгоритма и Big O нотации является
            важным для проектирования и реализации эффективных алгоритмов,
            которые могут экономить время и вычислительные ресурсы при решении
            сложных задач.
          </SubPara>
        </Paragraph>
        ...
        <Paragraph title="4. РЕКУРСИЯ" id="recursion">
          ...
          <SubPara>
            ...
            сложной проблемой, пока проблему нельзя решить без рекурсии.
            Рекурсия часто используется в алгоритмах, которым требуется
            повторное выполнение процесса с другим входом или набором входных
            данных каждый раз.
          </SubPara>
          <SubPara>
            Существуют две основные части рекурсивной функции: базовый случай,
            который определяет условие остановки (когда функция больше не
            вызывает себя), и рекурсивный случай (где функция вызывает саму себя
            с обновленными входными параметрами).
          </SubPara>
          <SubPara>
            ...
            вычислений, что и предыдущий шаг. Она часто используется в
            структурах данных, таких как деревья и графы, где структура данных
            естественным образом рекурсивна.
          </SubPara>
        </Paragraph>
        <Paragraph title="5. АЛГОРИТМЫ ПОИСКА" id="searching">
          <SubPara>
            Алгоритмы поиска в массиве - это набор техник и методов, которые
            используются для поиска определенных элементов или значений в
            массиве или списке элементов. Существует несколько популярных
            алгоритмов поиска в массиве, которые обычно используются, включая
            линейный поиск, двоичный поиск, интерполяционный поиск и
            экспоненциальный поиск.
          </SubPara>
          <SubPara>
            1. Линейный поиск: это простейший алгоритм поиска, который ищет
            каждый элемент массива или списка и сравнивает его с целевым
            элементом до тех пор, пока не будет найдено соответствие. Линейный
            поиск имеет временную сложность O(n), где n - размер массива.
          </SubPara>
          <SubPara>
          ...
          <SubPara>
            ...
            распределены. Он использует формулу для оценки местоположения
            целевого элемента в массиве, а затем выполняет двоичный поиск.
            Интерполяционный поиск имеет временную сложность O(log log n), что
            быстрее, чем двоичный поиск.
          </SubPara>
          <SubPara>
            4. Экспоненциальный поиск: это гибридный алгоритм, который
            объединяет как линейный, так и двоичный поиск. Он начинает поиск,
            рассматривая первый элемент, а затем увеличивает интервал в степени
            двойки до тех пор, пока не будет найден целевой элемент.
            Экспоненциальный поиск имеет временную сложность O(log n), что также
            совпадает с двоичным поиском.
          </SubPara>
          <SubPara>
        ...
        <Paragraph title="9. ХЭШ-ТАБЛИЦЫ И ХЭШ-ФУНКЦИИ" id="hashmap">
          <SubPara>
            Хеш-таблицы - это тип структуры данных, позволяющий эффективно
            хранить и извлекать пары {'"ключ-значение"'}. Они используют
            хеш-функцию для отображения каждого ключа в индекс массива, что
            позволяет мгновенно находить значения. Это делает хеш-таблицы
            полезными для задач, где важно быстрое время поиска, таких как
            кэширование, индексирование и операции с базами данных. Однако могут
            возникать коллизии, когда два разных ключа отображаются на один и
            тот же индекс, что требует дополнительных шагов для разрешения
            конфликтов. В целом, хеш-таблицы предоставляют мощный инструмент для
