    ...
    <Box>
      <Paragraph title="ОСНОВНЫЕ УЧЕБНЫЕ РЕСУРСЫ" id="resources">
        <SubPara>
          ...
          Питона, но большинство из них сосредоточены на ненужных нам
          темах. Что нам нужно для олимпиады - это основы
          программирования, синтаксис Python, знание его библиотек и
        ...
        <SubPara>
          ...
          , из которых я могу порекомендовать краш-курс от Гугла, но
          не бойтесь выбирать варианты, удобные лично для вас. Если
          вас интересуют курсы на русском языке, то вам может подойти
          курс Густокашина:{" "}
          <Span type="link" href="this">
        ...
        <SubPara>
          ...
          достижения успеха в олимпиадах объязательно нужно выше
          перечисленные темы. Сейчас мы как раз и познакомимся с этими
          темами и языком Python в целом. Не волнуйтесь, если что-то
          не поняли - все когда-то начинали, и после прохождения
          выбранных вами курсов, у вас сложится ясная цельная картина
        </SubPara>
      </Paragraph>
      <Paragraph title="1. УСТАНОВКА PYTHON И IDE" id="installation">
        ...
        <SubPara>
          ...
          Environment) для разработки на Python, но новичкам лучше
          подойдет VS Code, так как он бесплатный и лёгкий, а также на
          него можно установить расширения, которые вам помогут. VS
          Code можно скачать здесь:{" "}
          <Span type="link" href="this">
        ...
        <SubPara>
          ...
          PowerShell или Command Line). Основное отличие между
          работами в IDE и консолью, помимо удобства и внешнего вида
          является то, что в IDE мы пишем целую программу, состоящую
          из множества команд. А в консоли мы даем компьютеру
          отдельные команды и сразу получаем ответ:
        </SubPara>
        <CodeSection>
          <CodeLine>
      ...
      <Paragraph title="2. НАЗНАЧЕНИЕ ПЕРЕМЕННЫХ" id="variables">
        <SubPara>
          ...
          понятнее и можем присваивать ей любые значения, а не только
          числа. Например, если мы хотим хранить имя пользователя, мы
          назовём переменную <Span type="var-blue">user_name</Span>.
        </SubPara>
        <SubPara>
          Чтобы обозначить переменную в Python, надо поставить знак
          равно между названием переменной и её значением:
        </SubPara>
        <CodeSection>
      ...
      <Paragraph title="3. БАЗОВЫЕ ТИПЫ ДАННЫХ" id="data-types">
        <SubPara>
          Дело в том, что переменная в Питоне может принимать разные
          значения - строки, числа, символы и многое другое. Чтобы
          отличать тип строк от других, вокруг них надо ставить
          кавычки.
        </SubPara>
        <SubPara>
          ...
          <Box component="ol" sx={{ pl: 3 }}>
            ...
            <li>
              <Span type="highlight">None</Span> - отсутсвие данных
            </li>
          </Box>
        </SubPara>
        <CodeSection>
      ...
      <Paragraph
        title="4. МАТЕМАТИЧЕСКИЕ ОПЕРАТОРЫ"
      ...
      >
        ...
        <SubPara>
          У нас получится тоже самое, что и при обычном сложении этих
          чисел:
        </SubPara>
        <CodeSection>
          <CodeLine>
        ...
        <SubPara>
          Проводить эти операции можно так же, как и в математике - с
          помощью математических операторов:{" "}
      ...
      <Paragraph title="5. ПРОЦЕДУРЫ И ФУНКЦИИ" id="functions">
        ...
        <SubPara>
          ...
          отсутствием кодового слова <Span>return</Span>. В Питоне
          принято писать в таком случае слово <Span>pass</Span>:
        </SubPara>
        <CodeSection>
          <CodeLine>
      ...
      <Paragraph title="6. ОПЕРАТОРЫ УСЛОВИЯ" id="conditions">
        ...
        <SubPara>
          ...
          условия - тоже операции, возвращающие значение True или
          False:
        </SubPara>
        <CodeSection>
          <CodeLine type="centered">
      ...
      <Paragraph title="7. ОПЕРАТОРЫ ЦИКЛА" id="cycles">
        ...
        <SubPara>
          Например, если мы хотим досчитать до 100, то вместо того,
          чтобы вручную писать команду <Span>print</Span> для каждого
          числа, можно просто запустить цикл и добавлять к начальному
          числу по единице, пока оно не станет ровно 100:
        </SubPara>
        <CodeSection>
          <CodeLine>
        ...
        <SubPara>
          Для создания цикла в Питоне, нужно использовать кодовые
        ...
        <SubPara>
          ...
          количество мы передаем с встроенной функцией{" "}
          <Span>range</Span>:
        </SubPara>
        <CodeSection>
          <CodeLine>
      ...
      <Paragraph title="8. СИНТАКСИС PYTHON" id="syntax">
        <SubPara>
          Можно заметить, что назначения функций, условий и циклов
          внешне сходятся в том, что их содержимое сдвинуто вправо, а
          после названия функции, условия или цикла стоит знак
          двоеточия (<Span type="highlight bold">:</Span>). Если их
          убрать, то Python выдаст ошибку и программа не будет
          работать:
        </SubPara>
        <CodeSection>
        ...
        <SubPara>
          ...
          какое-нибудь объяснение своему коду, перед ним надо
          поставить хэштег (<Span type="highlight bold">#</Span>),
        ...
        <SubPara>
          ...
          писания на Питоне, вы научитесь ставить их автоматически.
        </SubPara>
      </Paragraph>
      ...
      <Paragraph title="ЗАКЛЮЧЕНИЕ">
        <SubPara>
          Ну вот и все, на этом пока можно закончить изучение Питона
          (если вы прошли выбранный вами курс). Понимания этих тем
          должно хватить для олимпиады, но не стоит останавливаться на
          достигнутом, продолжайте практикаваться и развиваться, ведь
          от этого шансы успеха на олимпиаде только увеличатся
        </SubPara>
      </Paragraph>
    </Box>
  );
